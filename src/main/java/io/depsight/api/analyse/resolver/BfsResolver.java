...
public class BfsResolver {
     ...
     * @return
     */
    public ResolutionResult resolve(List<ParsedDependency> directDeps, int maxDepth) {
        int cappedDepth = Math.min(maxDepth, MAX_ALLOWED_DEPTH);
        Set<String> visited = new HashSet<>();
        ConcurrentHashMap<String, List<VersionRequest>> versionCollector = new ConcurrentHashMap<>();

        List<DependencyNode> tree = Flux.fromIterable(directDeps)
                .flatMap(dep -> resolveNode(dep, 0, cappedDepth, visited, versionCollector))
                .collectList()
                .timeout(Duration.ofSeconds(15))
                .block();
        return new ResolutionResult(tree, versionCollector);
    }
    ...
    private Mono<DependencyNode> resolveNode(
        ...
        // The depth cap ()
        if (depth >= maxDepth) {
            return Mono.just(new DependencyNode(
                    dependency.groupId(),
        ...
        return mavenCentralClient
                ...
                .flatMap(pomXml -> {
                    ...
                    List<ParsedDependency> childrenDeps = PomParser.extractDependencies(model, properties);

                    List<MavenCooridinates> extractedImports = PomParser.extractImports(model, properties);

                    return parentBomResolver
                            .resolveBomImports(extractedImports, properties)
                            .flatMap(resolvedImports -> {
                                ...
                                for (ParsedDependency dep : childrenDeps) {
                                    if (dep.version().equals(UNRESOLVED)) {
                                        String version = resolvedImports.get(dep.groupId() + ":" + dep.artifactId());
                                        if (version != null) {
                                            resolvedChildrenDeps.add(new ParsedDependency(
                                ...
                                Mono<List<DependencyNode>> childrenMono = Flux.fromIterable(resolvedChildrenDeps)
                                        .flatMap(childDep ->
                                                resolveNode(childDep, depth + 1, maxDepth, visited, versionCollector))
                                        .collectList();

                                return childrenMono.map(children -> new DependencyNode(
                                        dependency.groupId(),
                                        dependency.artifactId(),
                                        dependency.version(),
                                        dependency.scope(),
